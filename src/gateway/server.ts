...
import { LMStudioRuntimeEstimator } from "../context/runtime-estimator/lmstudio.js"
...
export interface RequestMetrics {
  ...
  tokenEstimatorMode?: "shadow" | "runtime"
  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (incomingBody?.length && chatCompletion) {
        ...
        if (config.tokenEstimatorMode === "shadow" || config.tokenEstimatorMode === "runtime") {
          ...
          try {
            ...
            metrics.tokenEstimatorMode = config.tokenEstimatorMode
            
            // Se mode for runtime, e a estimativa falhar nao estara disponivel (fallback automatico)
            if (config.tokenEstimatorMode === "runtime") {
              metrics.requestTokensBefore = runtimeEstimateTokens
              metrics.estimatorConfidence = "exact"
            }
          } catch (e) {
